...
data class Value(val textualValue: String) {
    ...
    private var resolvedValue = Float.NaN
            ...
            for (o in ops) {
                if (tokens[i] == o) {
                    ...
                    if (o.requireRightValue() && o.requireLeftValue()) {
                        val a = tokens[i - 1] as? Float
                                ?: throw IllegalArgumentException("Chained operators: $o and ${tokens[i - 1]}")
                        val b = tokens[i + 1] as? Float
                                ?: throw IllegalArgumentException("Chained operators: $o and ${tokens[i + 1]}")
                        // perform and add
                        val v = o.execute(a, b)
                        // push to swap
 ...
 */
fun findIndex(vararg objs: Any, list: ArrayList<Any>) = list.indices.firstOrNull { i -> list[i] == objs[0] && objs.indices.none { list[i + it] != objs[it] } }
        ?: -1